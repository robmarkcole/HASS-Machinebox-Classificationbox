...
from http import HTTPStatus #common HTTP Lib for statuses as per HA changes
...
def post_image(url, image, username, password):
    ...
    input_json = {
        "inputs": [
            {"key": "image", "type": "image_base64", "value": encode_image(image)}
        ]
    }
    try:
        response = requests.post(url, json=input_json, **kwargs)
        return response
    except requests.exceptions.ConnectionError:
        _LOGGER.error("ConnectionError: Is %s running?", CLASSIFIER)
        return None
...
class ClassificationboxEntity(ImageProcessingEntity):
    ...
    def __init__(
        ...
        self._name = "{} {} {}".format(CLASSIFIER, camera_name, model_name)
        self._state = None
        self._matched = {}
    ...
    def process_image(self, image):
        ...
        response = post_image(predict_url, image, self._username, self._password)
